...
export default function SkillSettings() {
  ...
  useEffect(() => {
    ...
    setEnabled(settings.skill_enabled);
    setListMax(settings.skill_list_max);
  }, [settings]);
  ...
  const submit = () => {
    ...
    updateMutation.mutate(
      { skill_enabled: enabled, skill_list_max: listMax },
      {
        onSuccess: () => {
          setSaveMsg(t('skill.settings.saved'));
        },
        onError: (err) => {
          setSaveError(t('skill.saveError', { error: getApiErrorMessage(err) }));
        },
      },
    );
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-row flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-muted-foreground">{t('skill.settings.enabledDesc')}</p>
        <div className="flex items-center gap-2">
          <Switch
            checked={enabled}
            onCheckedChange={setEnabled}
            aria-label={t('skill.settings.enabled')}
          />
          ...
          <>
            <div className="flex flex-wrap items-end gap-4">
              <div className="w-40 space-y-2">
                <Label>{t('skill.settings.listMax')}</Label>
                <Input
                  type="number"
                  min={1}
                  value={listMax}
                  onChange={(e) => setListMax(Number(e.target.value))}
                  aria-label={t('skill.settings.listMax')}
                />
              </div>
            </div>
