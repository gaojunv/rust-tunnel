...
/** pre 组件覆盖：Streamdown 默认 pre 只是 cloneElement 透传。`code` 高亮插件
 *  （@streamdown/code，基于 Shiki）已移除——代码块无语法高亮，code 内是纯文本。
 *  自己画单层框：语言头（左语言名，右官方 CodeBlockCopyButton）+ 透传 code 本体。 */
...
/** 助手消息 Markdown 渲染：Streamdown（流式半截语法容错），
 ...
 *
 *  注：`code` 高亮插件（@streamdown/code，基于 Shiki）已移除，代码块无语法
 *  高亮（保留语言标签 + 复制按钮）；`streaming` prop 仅保留以兼容旧调用方。 */
const MD_CLASS = [
  'text-sm leading-7',
...
export default memo(function Markdown({
  ...
  // streaming 仅为兼容保留：高亮插件移除后无流式降级分支，接受该参数避免调用方改动
  void streaming;
  return (
    <Streamdown
      className={MD_CLASS}
