...
pub async fn handle_chat_completions(
    ...
    let prepared = super::pipeline::PreparedRequest {
        ...
        anthropic_body: None,
        opencode_session: super::pipeline::extract_opencode_session(&headers, &body),
    };
...
mod tests {
    ...
    async fn test_stream_rewrite_is_incremental() {
        ...
        while let Some(item) = out.next().await {
            tail.push_str(core::str::from_utf8(&item.unwrap()).unwrap());
        }
        assert!(tail.contains("[DONE]"), "流应正常收尾: {tail}");
    }
