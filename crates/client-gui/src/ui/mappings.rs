    ...
    let Some(summary) = &status.mapping_summary else {
        egui::Frame::group(ui.style())
            ...
            .show(ui, |ui| {
                ...
                ui.add_space(4.0);
                ui.label(
                    egui::RichText::new("暂无映射摘要（等待服务端推送或该客户端未被任何规则引用）")
                        .weak(),
                );
            });
        return;
        ...
        .show(ui, |ui| {
            ...
            for rule in &summary.rules {
                egui::CollapsingHeader::new(format!(
                    "{} — {}  [{}]",
                    rule.name, rule.listen, rule.id
                ))
                .default_open(false)
                .show(ui, |ui| {
                    if !rule.domains.is_empty() {
                        ui.label(
                            egui::RichText::new(format!("域名: {}", rule.domains.join(", ")))
                                .small(),
                        );
                    }
                    ui.label(
                    ...
                    for route in &rule.routes {
                        ui.add_space(4.0);
                        ui.label(
                            egui::RichText::new(format!("路径: {}", route.path))
                                .small()
                                .strong(),
                        );
