    ...
    let result = match challenge_type.as_str() {
        "dns-01" => {
            ...
            match dns_config.as_ref() {
                Some(config) => {
                    let solver: Arc<dyn crate::acme::dns::DnsChallengeSolver> = match config
                        .provider
                    {
                        crate::acme::dns::DnsProvider::Aliyun => Arc::new(
                            crate::acme::dns::aliyun::AliyunDnsSolver::new(config),
                        ),
                        crate::acme::dns::DnsProvider::Cloudflare => Arc::new(
                            crate::acme::dns::cloudflare::CloudflareDnsSolver::new(config),
                        ),
                        crate::acme::dns::DnsProvider::TencentCloud => Arc::new(
                            crate::acme::dns::tencent::TencentDnsSolver::new(config),
                        ),
                        crate::acme::dns::DnsProvider::Custom => Arc::new(
                            crate::acme::dns::custom::CustomDnsSolver::new(config),
                        ),
                    };
...
pub async fn update_acme_config(
    ...
    if should_init_client {
        ...
        if let Some(db) = state.server_state.get_db() {
            let acme_state = crate::acme::AcmeState::with_db(db.clone());
            let client = Arc::new(crate::acme::client::AcmeClient::new(
                acme_state,
