...
mod tests {
    ...
    fn test_truncate_summary_short_unchanged() {
        ...
        assert_eq!(subagent::truncate_summary(text.clone()), text);
    }

    #[test]
    fn test_truncate_summary_long_truncated() {
        let text = "x".repeat(TASK_SUMMARY_MAX_CHARS + 100);
        let result = subagent::truncate_summary(text);
    ...
    fn test_subagent_compact_messages_clears_old_tool_msgs() {
        ...
        let did_compact = subagent::subagent_compact_messages(&mut rt);
        assert!(did_compact);
        // 前 4 条应被清空，后 4 条保留
        assert_eq!(
    ...
    fn test_clone_sub_rt_copies_key_fields() {
        ...
        let cloned = subagent::clone_sub_rt(&rt);
