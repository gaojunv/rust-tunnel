...
pub(crate) const DEDUP_THRESHOLD: f32 = 0.90;
/// 去重检索的候选数（同作用域过滤前 over-fetch 的量）。
pub(crate) const DEDUP_TOP_K: usize = 3;
/// 单条记忆 content 上限（与 remember schema 的 ≤2048 一致）。
pub(crate) const MEMORY_CONTENT_MAX_CHARS: usize = 2048;
/// tags 数量上限。
...
pub(crate) fn scope_coords(scope: &str, client_id: &str, workspace_id: &str) -> (String, String, String) {
    match scope {
...
pub(crate) async fn upsert_memory_with_dedup(
    memory: &MemoryState,
...
pub(crate) fn test_store() -> (tempfile::TempDir, VectorStore) {
...
#[cfg(all(test, feature = "rag"))]
pub(crate) async fn mock_embedding_server(dim: usize) -> String {
...
#[cfg(all(test, feature = "rag"))]
pub(crate) async fn test_memory_with_embedding(base_url: &str) -> (Database, MemoryState) {
