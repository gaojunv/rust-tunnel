...
//! 提炼原子事实 + 可复用 Skill，再落库。事实逐条向量化 + 同作用域去重；
//! Skill **不向量化**（纯 SQLite 按 name+scope 文本去重，embedding 未配置也能落库）。
//!
//! 防重：`agent_sessions.distilled` 列的原子 CAS（`memory_mark_distilled_if_not`）
//! 保证多路并发触发（archive + delete + disconnect + idle）只有唯一赢家。
//!
//! 门闸：`enabled != 0 || skill_enabled != 0`（skill_enabled=1 单独开启也能触发
//! 蒸馏——Skill 蒸馏仅需 LLM，不依赖 embedding）。
//!
//! 防蒸馏回环：蒸馏渲染时剥离 `<memory>...</memory>`、`<skills>...</skills>` 块与
//! @引用文件包装块，已注入的记忆/技能清单不会被当成对话内容再次蒸馏。
...
async fn do_distill(memory: MemoryState, snapshot: DistillSnapshot, trigger: String) {
    ...
    let value = match parse_distill_value(&text) {
        Ok(v) => v,
        Err(e) => {
    ...
    let facts = match facts_from_value(&value) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(session_id = %snapshot.session_id, error = %e, "distill facts parse failed");
            send_failed(&memory, &snapshot.session_id);
            return;
        }
    };
    // skills 字段缺失 → 空列表不报错（向后兼容，e2e mock 零改动）；单条非法跳过。
    let skills = skills_from_value(&value);

    // 记忆事实：emb 未配置时跳过（配置问题属预期路径，不广播 failed）。Skill
    // **不依赖 embedding**——即便 emb 缺失、skill_enabled=1，仍照常落库。
    ...
    if s.skill_enabled != 0 {
        for skill in skills {
            ...
            .await
            {
                Ok(_) => skills_saved += 1,
                Err(e) => {
                    tracing::warn!(session_id = %snapshot.session_id, error = %e, "distill upsert skill failed");
                }
            }
        }
    }
...
/// 总闸 + CAS 前置 enabled 检查。记忆（enabled）与 Skill（skill_enabled）任一开启
/// 即触发蒸馏——Skill 蒸馏仅需 LLM，不依赖 embedding。
async fn memory_enabled(memory: &MemoryState) -> bool {
    let s = memory.settings().await;
    s.enabled != 0 || s.skill_enabled != 0
}
...
另：若会话含可复用的执行流程/方法论（排障手册、发布清单、评审步骤、部署脚本套路等），另输出 \"skills\" 数组，至多 3 条。每条：name（动词短语，≤32 字符）、description（触发边界 + 概述，只凭这条判断何时调用，≤200 字符）、content（Markdown 全文 800~2000 字，含前置条件/执行步骤/具体命令/验证方法）、scope（workspace|client|global，规则同上）、tags（最多 3 个）。无合适内容时**省略 skills 字段**：
{\"facts\":[...],\"skills\":[{\"name\":\"...\",\"description\":\"...\",\"content\":\"...\",\"scope\":\"workspace\",\"tags\":[\"...\"]}]}";
...
/// 一条待落库的蒸馏 Skill（可复用执行经验）。`name`/`content` 已校验非空且
...
/// 解析蒸馏 LLM 输出为 serde Value。支持代码围栏（```json ... ```）、裸对象，
/// 并对围栏内外的说明文字鲁棒（取首 `{` 到末 `}` 的子串）。
fn parse_distill_value(raw: &str) -> Result<serde_json::Value, String> {
    let cleaned = extract_json_object(&strip_code_fence(raw));
    serde_json::from_str(&cleaned).map_err(|e| format!("invalid distill JSON: {e}"))
}

/// 从文本中提取首 `{` 与末 `}` 之间的子串（部分模型会在 JSON 前后附加说明文本）。
/// 两者缺一或顺序倒置 → 原样返回（让 serde 给出更准确的错误）。
...
/// 从已解析的 Value 提取事实列表。缺 facts 数组 / 全部条目非法 → Err（沿用既有
/// 语义）；单条非法（空 content/超长）跳过。
fn facts_from_value(value: &serde_json::Value) -> Result<Vec<DistillFact>, String> {
    let arr = value
        .get("facts")
...
/// 从已解析的 Value 提取 Skill 列表。`skills` 字段缺失 → 空列表不报错（向后兼容，
/// e2e mock 零改动）；单条非法（name 空/超长、content 空/超长）跳过；description
/// 截 200；scope 归一化；总量 ≤ `DISTILL_SKILL_MAX`。
fn skills_from_value(value: &serde_json::Value) -> Vec<DistillSkill> {
    let Some(arr) = value.get("skills").and_then(|v| v.as_array()) else {
        return Vec::new();
    };
    let mut skills = Vec::new();
    for item in arr {
        if skills.len() >= DISTILL_SKILL_MAX {
            break;
        }
        let name = item
            .get("name")
...
/// 保留的薄包装：兼容既有单测与调用方（`parse_facts(&str) -> Result<Vec<DistillFact>>`）。
/// 内部走 `parse_distill_value` + `facts_from_value` 管道。生产路径（`do_distill`）
/// 直接复用 `parse_distill_value` 的 Value（还要提取 skills），故仅测试使用。
#[cfg(test)]
fn parse_facts(raw: &str) -> Result<Vec<DistillFact>, String> {
    let value = parse_distill_value(raw)?;
    ...
    for m in messages {
        match (m.role.as_str(), m.tool_calls.as_deref()) {
            ("tool", _) => {
                ...
                // 防回环：use_skill 的全文结果（skill content）不回灌给蒸馏 LLM——
                // 只渲染单行占位，避免 skill 全文被当成会话内容再次蒸馏出重复 skill。
                let content = if name == "use_skill" {
                    "[skill content loaded]".to_string()
                } else {
                    // M2 起 ACP 路径落库 content 为 JSON `{"text","status",...}`——
                    // 蒸馏渲染只取 text（否则把 JSON 壳喂给蒸馏 LLM）；旧纯文本行
                    // 原样使用（tool_result_text 返回 None）。
                    crate::agent::tool_result::tool_result_text(&m.content)
...
mod tests {
    ...
    #[test]
    fn parse_facts_extracts_json_from_wrapped_text() {
        // 模型在 JSON 前后附加说明文本（非代码围栏）：取首 { 末 } 应仍能解析。
        let raw = "好的，这是提炼结果：\n{\"facts\": [{\"content\": \"用户偏好简洁\", \"scope\": \"global\"}]}\n希望对你有帮助。";
        let facts = parse_facts(raw).unwrap();
        assert_eq!(facts.len(), 1);
        assert_eq!(facts[0].content, "用户偏好简洁");
        assert_eq!(facts[0].scope, "global");
        // 无花括号 → 保持原样报错
        assert!(parse_facts("完全没有 JSON 的花括号").is_err());
    }

    #[test]
    fn parse_skills_missing_field_is_empty() {
        // 旧 mock 只返回 facts：skills 字段缺失 → 空列表不报错（向后兼容）。
        let value = serde_json::json!({"facts": [{"content": "x", "scope": "workspace"}]});
        assert!(skills_from_value(&value).is_empty());
        // skills 非数组 → 空
        assert!(skills_from_value(&serde_json::json!({"skills": "nope"})).is_empty());
    }

    #[test]
    fn parse_skills_valid_and_invalid_entries() {
        let value = serde_json::json!({
            "skills": [
                {"name": "发布应用", "description": "发布前检查与回滚", "content": "## 步骤\n1. build\n2. deploy", "scope": "workspace", "tags": ["deploy"]},
                {"name": "  ", "content": "空 name 应跳过", "scope": "workspace"},                       // 空 name 跳过
                {"name": "超长内容", "content": "x".repeat(SKILL_CONTENT_MAX_CHARS + 1), "scope": "workspace"}, // 超长 content 跳过
                {"name": "超长描述", "description": "d".repeat(500), "content": "ok", "scope": "bogus"}   // 描述截 200、scope 回落 workspace
            ]
        });
        let skills = skills_from_value(&value);
        assert_eq!(skills.len(), 2, "skills: {skills:?}");
        assert_eq!(skills[0].name, "发布应用");
        assert_eq!(skills[0].description, "发布前检查与回滚");
    ...
    #[test]
    fn parse_skills_caps_at_three() {
        let skills_json: Vec<serde_json::Value> = (0..6)
            .map(|i| serde_json::json!({"name": format!("skill-{i}"), "content": "ok", "scope": "global"}))
            .collect();
        let value = serde_json::json!({"skills": skills_json});
        let skills = skills_from_value(&value);
        assert_eq!(skills.len(), DISTILL_SKILL_MAX, "至多 3 条");
    }

    #[test]
    fn render_strips_skills_block() {
        let msgs = vec![msg(
            "assistant",
            "好的\n<skills>\n- skill-a: 描述\n</skills>\n继续",
            "message",
    ...
    #[test]
    fn render_use_skill_tool_placeholder() {
        // use_skill 全文结果不回灌：渲染单行占位
        let big = "## 排障手册\n".to_string() + &"x".repeat(2000);
        let mut t = tool_msg(&big);
        t.name = Some("use_skill".into());
        let out = render_distill_text(&[t]);
        assert!(out.contains("tool(use_skill)"));
        assert!(out.contains("[skill content loaded]"));
        assert!(!out.contains("排障手册"), "skill 全文不应回灌");
    }
    ...
    #[tokio::test]
    async fn distill_gate_passes_when_only_skill_enabled() {
        ...
        // skill_enabled=1、enabled=0、emb 未配置
        let mut settings = db.memory_get_settings().await.unwrap();
        settings.skill_enabled = 1;
        db.memory_upsert_settings(&settings).await.unwrap();
        ...
        let ev = tokio::time::timeout(std::time::Duration::from_secs(5), rx.recv())
            .await
            .expect("running event (gate should pass)")
            .expect("channel closed");
        assert_eq!(ev.status, "running");
        let ev = tokio::time::timeout(std::time::Duration::from_secs(5), rx.recv())
            .await
