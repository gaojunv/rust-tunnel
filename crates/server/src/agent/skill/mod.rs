...
pub(crate) async fn upsert_skill_with_dedup(
    memory: &crate::agent::memory::MemoryState,
    name: &str,
    description: &str,
    content: &str,
    scope_type: &str,
    ...
    if let Some(existing) = existing {
        let merged = crate::agent::memory::merge_tags(&crate::agent::memory::parse_tags(&existing.tags), tags);
...
mod tests {
    ...
    async fn memory_state() -> (Database, MemoryState) {
        ...
        let (_dir, store) = crate::agent::memory::test_store();
    ...
    async fn upsert_dedup_updates_same_scope_keeps_use_count() {
        ...
        let tags = crate::agent::memory::parse_tags(&row.tags);
