...
mod tests {
    ...
    fn last_user_message_extracted() {
        let msgs = vec![
            crate::llm::ChatMessage::text("system", "s"),
            crate::llm::ChatMessage::text("user", "第一句"),
            crate::llm::ChatMessage::text("assistant", "答"),
            crate::llm::ChatMessage::text("user", "第二句"),
        ];
    ...
    fn inject_prepends_system_message() {
        let mut msgs = vec![crate::llm::ChatMessage::text("user", "问")];
