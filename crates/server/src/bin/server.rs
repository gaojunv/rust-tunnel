...
async fn disable_conflicting_rules_on_port(
    ...
    listen_addr: &str,
    err: &rust_tunnel_server::reverse_proxy::error::ReconcileError,
) {
    use rust_tunnel_server::reverse_proxy::error::ReconcileError as E;
    use rust_tunnel_server::reverse_proxy::{ProxyRule, RuleType};
...
async fn main() -> TunnelResult<()> {
    ...
    let dynamic_config =
        rust_tunnel_server::dynamic_config::DynamicConfig::load_or_seed(&db, &config).await;
    ...
    *state.acme.full_config.write().await = effective_acme_config.clone();
    ...
    if config.dns_enabled {
        let dns_registry = rust_tunnel_server::DnsRegistry::new(
            "0.0.0.0".to_string(),
    ...
    if effective_acme_config.enabled {
        let acme_state = rust_tunnel_server::acme::AcmeState::with_db(db.clone());
        let client = rust_tunnel_server::acme::client::AcmeClient::new(
            acme_state,
        ...
        let cert_manager = Arc::new(rust_tunnel_server::acme::manager::CertificateManager::new(
            &effective_acme_config.cert_dir,
        ));
    ...
    let llm_master_key =
        match rust_tunnel_server::llm::crypto::load_or_create_master_key(&config.db_path) {
            Ok(k) => Some(k),
            Err(e) => {
                tracing::warn!(
    ...
    let control_tls_rx = if config.tls {
        ...
        if let Some(ref cert_manager) = state.acme.cert_manager {
            ...
            tokio::spawn(async move {
                use rust_tunnel_server::acme::manager::CertEvent;
                loop {
    ...
    // Start Trojan if enabled in dynamic config
    {
        ...
        if let Some(ref tj) = tj {
            if let Err(e) =
                rust_tunnel_server::trojan_runtime::apply_trojan_config(&state, tj).await
            {
