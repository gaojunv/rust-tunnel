...
//!
//! 独立 crate（批次 6 拆分），依赖 llm + persistence + common。
//! rag feature 门控 memory/skill/wiki/mcp 子系统。

// 测试代码豁免 panic 风险 lint（生产代码仍告警）
#![cfg_attr(test, allow(clippy::unwrap_used, clippy::expect_used, clippy::panic))]

// re-export 使 agent 内部 `crate::llm` / `crate::db` 路径无需逐文件修改
...
// ── 隧道执行抽象（依赖倒置接缝）────────────────────────────────────────────
//
// agent crate 不依赖 control_plane 的具体 `ClientRegistry`；通过 `TunnelExecutor`
// trait 消费通道原语，server 装配层用 `ClientRegistry` 适配实现。

/// `client_handle` 的返回载体：agent 侧只消费版本号（功能门控）与控制通道
/// sender（AcpBridge 回发 `AgentLlmProxyChunk`）。
#[derive(Debug, Clone)]
pub struct ClientHandle {
    /// 客户端上报的版本号（`client_supports_cancel` 等功能门控用）。
    pub client_version: Option<String>,
    /// 控制通道下行 sender（服务端 → 客户端）。
    pub control_sender: mpsc::Sender<rust_tunnel_common::ControlMessage>,
}

/// agent 对隧道控制通道的能力抽象。
///
/// 方法面对应 `ClientRegistry` 的五个通道原语；server 装配层以
/// `impl TunnelExecutor for ClientRegistry` 接入，测试以内存 mock 接入。
#[async_trait::async_trait]
pub trait TunnelExecutor: Send + Sync {
    /// 查询客户端句柄（版本号 + 控制通道 sender）；客户端离线返回 None。
    async fn client_handle(&self, client_id: &str) -> Option<ClientHandle>;

    /// 发送协商请求并等待以 session_id 匹配的响应
    /// （`AgentSpawnResponse` / `AgentLlmProxyReady`）。
    ///
    /// # Errors
    /// - `NotConnected` — client offline
    /// - `BrokenPipe` — control channel closed / responder dropped
    /// - `TimedOut` — no response within `timeout`
    async fn spawn_negotiate(
        &self,
        client_id: &str,
        session_id: &str,
        request: rust_tunnel_common::ControlMessage,
        timeout: Duration,
    ) -> std::io::Result<rust_tunnel_common::ControlMessage>;

    /// 经控制通道在客户端执行一条 agent 命令。
    ///
    /// # Errors
    /// 同 `spawn_negotiate`；超时实现会尽力下发 `AgentExecCancel` 真杀客户端侧
    /// 残留进程（仅对支持 cancel 的客户端版本）。
    ...
    /// 向客户端下发 `AgentExecCancel`；客户端离线静默返回 false。
    ...
    async fn open_tunnel(
        ...
        client_id: &str,
        target_addr: &str,
    ) -> std::io::Result<TunnelByteStream>;
}
...
pub struct AgentState {
    ...
    /// 默认 [`crate::github::GITHUB_API_BASE`]）。
...
impl AgentState {
    pub fn new(registry: Arc<dyn TunnelExecutor>, db: Database) -> Self {
        ...
        let mut state = Self {
            ...
            github_base_url: crate::github::GITHUB_API_BASE.to_string(),
        };
    ...
    #[cfg(test)]
    pub async fn pending_elicitations_count(&self) -> usize {
...
pub(crate) mod test_helpers {
    ...
    use super::{ClientHandle, TunnelByteStream, TunnelExecutor};

    /// 测试用客户端注册表：模拟 `ClientRegistry` 的协商配对核心逻辑。
    ///
    /// `register` 存入控制通道 sender；`spawn_negotiate` 发送请求后在
    /// `spawn_pending` 里等响应；`resolve_spawn_pending` 完成配对。
    /// 测试用 `mpsc` 通道从客户端侧收消息并自动应答。
    #[derive(Clone, Debug)]
    ...
    #[derive(Clone, Debug)]
    struct TestClientEntry {
        client_version: Option<String>,
        control_sender: mpsc::Sender<ControlMessage>,
    }

    impl TestRegistry {
        pub(crate) fn new(_db: &Database) -> Self {
            Self {
                entries: Arc::new(tokio::sync::RwLock::new(HashMap::new())),
                spawn_pending: Arc::new(Mutex::new(HashMap::new())),
                agent_pending: Arc::new(Mutex::new(HashMap::new())),
            }
        }

        /// 注册模拟客户端（`control_sender` 存入 entry，供 `spawn_negotiate` 发送）。
        ...
        /// 完成协商配对（模拟客户端侧应答）。
        pub(crate) async fn resolve_spawn_pending(
            &self,
            session_id: &str,
            msg: ControlMessage,
        ) -> bool {
        ...
        /// 完成 agent_exec 配对（模拟客户端侧应答 `AgentExecResult`）。
        pub(crate) async fn deliver_agent_response(
            &self,
            _client_id: &str,
            request_id: &str,
            result: rust_tunnel_common::AgentResult,
        ) {
            if let Some(tx) = self.agent_pending.lock().await.remove(request_id) {
                let _ = tx.send(result);
            }
        }
    }
    ...
    #[async_trait::async_trait]
    impl TunnelExecutor for TestRegistry {
        async fn client_handle(&self, client_id: &str) -> Option<ClientHandle> {
            self.entries
                .read()
                .await
                ...
                .map(|e| ClientHandle {
                    ...
                    control_sender: e.control_sender.clone(),
                })
        }
        ...
        async fn spawn_negotiate(
            ...
            let entry = entries
                .get(client_id)
                .ok_or_else(|| Error::new(ErrorKind::NotConnected, "client offline"))?;
            ...
            match send_result {
                ...
                Err(_) => {
                    self.spawn_pending.lock().await.remove(session_id);
                    return Err(Error::new(ErrorKind::TimedOut, "channel congested"));
                }
            }
            match tokio::time::timeout(timeout, rx).await {
                ...
                Err(_) => {
                    self.spawn_pending.lock().await.remove(session_id);
                    Err(Error::new(ErrorKind::TimedOut, "negotiate timeout"))
                }
            }
        }
        ...
        async fn agent_exec(
            ...
            if send_result.is_err() {
                self.agent_pending.lock().await.remove(request_id);
                return Err(Error::new(ErrorKind::BrokenPipe, "control channel closed"));
            }
            match tokio::time::timeout(timeout, rx).await {
                ...
                Err(_) => {
                    self.agent_pending.lock().await.remove(request_id);
                    Err(Error::new(ErrorKind::TimedOut, "agent exec timeout"))
                }
            }
        }
